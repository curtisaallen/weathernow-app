...
import { getWeatherData, getWeatherFor, getDailyForecast, getWeatherStats, getHourlyByDayFromCoords } from './actions/getData';
...
const handleSubmit: React.MouseEventHandler<HTMLButtonElement> = async (event) => {
  event.preventDefault();
          ...
          const fetchgetHourlyByDayFromCoords = await getHourlyByDayFromCoords(fetchedData.results[0].latitude, fetchedData.results[0].longitude, 7);
       ...
       <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-7 flex items-center justify-between">
          ...
          <div className="setting">
            <button className="bg-brand-deep hover:bg-brand-deep-dark text-white font-bold py-2 px-4 rounded flex items-center justify-between gap-2">
                ...
                <span>
                    <select  className="bg-transparent text-white font-bold outline-none appearance-none cursor-pointer">
  ...
  <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-9 text-center flex flex-col items-center">
    ...
    <form className="flex items-center justify-center mt-14 max-w-2xl justify-between gap-4">
        <div className="flex py-4 px-6 bg-brand-deep rounded justify-between gap-2">
            ...
            <input value={searchTerm} onChange={handleSearch} className="bg-transparent text-white outline-none appearance-none cursor-pointer" placeholder="Search for a place..." />
        </div>
        <button onClick={handleSubmit} className="bg-panel rounded text-white font-bold py-4 px-6">
            Search
        </button>
    </form>

    <div className="w-full py-16">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-[minmax(0,1fr)_350px]">
...
{days.length != 0 ? <h3 className="text-left text-[1rem] font-semibold">Daily forecast</h3>: '' }
                ...
                <div className="daily-forecast py-5 w-full flex flex-nowrap gap-2 items-stretch">
